...
for tr in trs:
    if(len(tr.select('td')) > 10) :
        ...
        retVal.append({
            'code':extractCode(tr)
            , 'name': extractName(tr)
            , 'current': extractNumber(tr, 2)
            , 'market_cap': extractNumber(tr, 6)
            , 'foreigner_rate': extractNumber(tr, 8)
            , 'volume': extractNumber(tr, 9)
            , 'PER': extractNumber(tr, 10)
            , 'ROE': extractNumber(tr, 11)
        })
